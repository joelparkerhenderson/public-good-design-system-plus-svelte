...
An angle slider input is a specialized range input component for selecting an angle
...
<AngleSliderInput label="Rotation" bind:value={angle} />
...
<AngleSliderInput label="Direction" bind:value={direction} step={15} />
```
...
```svelte
<AngleSliderInput label="Locked rotation" value={45} disabled />
```
...
```svelte
<form>
  <AngleSliderInput label="Gradient angle" bind:value={angle} name="gradient-angle" id="gradient" />
</form>
```
